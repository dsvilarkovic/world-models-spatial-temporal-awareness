...
def worker(worker_arg_tuple):
    ...
    for rollout_num in rollouts_per_core:
        ...
        filen = str(np.random.randint(sys.maxsize))+".npz"
        np.savez_compressed("data/rollout_doom/"+filen, frames=frames_array, actions=actions_array, rewards=rewards_array)
...
def main():
    ...
    parser.add_argument('--data_dir', '-d', default="/data/rollout_doom", help='The base data/output directory')
    ...
    parser.add_argument('--frame_resize', '-r', default=128, type=int, help='h x w resize of each observation frame')
