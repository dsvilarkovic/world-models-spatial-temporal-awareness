...
class RNN():
	...
	def _build(self):
		...
		rnn_x = Input(shape=(None, Z_DIM + ACTION_DIM + 1), name='rnn_x_input')
		...
		state_input_h = Input(shape=(HIDDEN_UNITS,), name='h_input')
		state_input_c = Input(shape=(HIDDEN_UNITS,), name='c_input')
	...
	def train(self, rnn_input, rnn_output, model_name):
		tensorboard_callback = TensorBoard(log_dir='./rnn/'+model_name+'/log/')
